...
Installing `python-annoy` from the `conda-forge` channel can be achieved by adding `conda-forge` to your channels with:

```
conda config --add channels conda-forge
```

Once the `conda-forge` channel has been enabled, `python-annoy` can be installed with:

```
conda install python-annoy
```

It is possible to list all of the versions of `python-annoy` available on your platform with:

```
conda search python-annoy --channel conda-forge
```
...
Updating python-annoy-feedstock
===============================

If you would like to improve the python-annoy recipe or build a new
...
Note that all branches in the conda-forge/python-annoy-feedstock are
